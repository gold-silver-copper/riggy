...
use crate::llm::{LlmBackend, WorldAction, build_request};
...
impl<B: LlmBackend> GameService<B> {
    ...
    async fn submit_dialogue_line(&mut self, input: String) -> Result<Vec<GameEvent>> {
        ...
        let response = self.backend.generate_dialogue(&request).await?;
        ...
        events.extend(self.apply_actions(npc_id, response.actions));
    ...
    fn start_dialogue(&mut self, npc_id: NpcId) -> Result<Vec<GameEvent>> {
        ...
        let opening_text = format!("What do you want to know about {}?", self.current_city().name);
    ...
    fn enter_vehicle(&mut self, entity_id: EntityId) -> Result<GameEvent> {
        ...
        Ok(GameEvent::VehicleEntered {
            entity: EntityRef { id: entity_id, name: vehicle.name, kind: vehicle.kind },
        })
    }
    ...
    fn exit_vehicle(&mut self) -> Result<GameEvent> {
        ...
        Ok(GameEvent::VehicleExited {
            entity: EntityRef { id: vehicle_id, name: vehicle.name, kind: vehicle.kind },
        })
    }
    ...
    async fn leave_dialogue(&mut self) -> Result<GameEvent> {
        let Some(session) = self.state.active_dialogue.take() else {
            bail!("You are not talking to anyone right now.");
        };
    ...
    fn apply_actions(&mut self, npc_id: NpcId, actions: Vec<WorldAction>) -> Vec<GameEvent> {
        let mut applied = Vec::new();
        for action in actions {
            let WorldAction::UpdateRelationship { delta, note } = action;
